...
1. Live demo URL: add your Vercel deployment URL here after deploy.
2. Source repository: add your GitHub/GitLab/Bitbucket URL here after push.
3. Architecture diagram: `docs/nexus-architecture.svg`.
4. Login credentials:
...
Run `supabase/schema.sql`, then `supabase/seed.sql` in Supabase SQL Editor. In Vercel, set the project root directory to `Nexus` and add `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `SESSION_SECRET`.
